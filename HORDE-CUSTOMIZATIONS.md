...
| 15 | claude-plugin/skills/wiki-manager/references/wiki-structure.md | schema | Two micro-changes that give the LLM positive instruction (do this) instead of trusting it to avoid drift on its own. (a) Tighten the `summary:` placeholder in both Source and Wiki Article schema examples from `"2-3 sentence summary"` / `"2-3 sentence summary for index"` to a single canonical phrase: `"2-3 sentence plain-prose summary, rendered verbatim in indexes"`. The phrase carries three implicit positives — plain prose, rendered verbatim, audience is index listings — so the model writes for the index renderer instead of leaking `[md](links.md)` or `[[wikilinks]]` into the field. (b) Append one sentence to the Tag Convention section pointing at the per-topic master vocabulary file: `When a topic ships wiki/references/tag-vocabulary.md, prefer tags from that table; add new tags there before using them in articles.` Per-topic instead of HUB-level so engagement-scoped tags stay client-isolated. No lint rule yet — enforcement lands as a separate patch once the convention has been validated against more ingest cycles. Tests: 34/34 plugin-validate, 92/92 structural pass unchanged. |
